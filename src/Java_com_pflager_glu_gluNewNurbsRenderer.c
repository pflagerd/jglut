...
JNIEXPORT jobject JNICALL Java_com_pflager_glu_gluNewNurbsRenderer(JNIEnv *env, jobject object) {
    DPRINTF("gluNewNurbsRenderer();\n");

    GLUnurbs* glunurbs = gluNewNurbsRenderer();

    jclass cls = (*env)->FindClass(env, "Lcom/pflager/glu$GLUnurbs;"); // /GLUnurbs
    DPRINTF("cls == %p\n", cls); 
    if (NULL == cls)
    	return NULL;
    ...
    (*env)->SetLongField(env, newObj, fidNumber, (jlong)glunurbs);
